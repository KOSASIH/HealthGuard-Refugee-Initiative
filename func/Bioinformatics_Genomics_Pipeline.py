...
def preprocess_genomic_data(genomic_data):
    ...
    genomic_data = genomic_data[genomic_data['Marker'].isin(['Marker1', 'Marker2', 'Marker3'])]
...
def dimensionality_reduction(genomic_data):
    ...
    pca_transformed_data = pca.fit_transform(genomic_data[['Marker1', 'Marker2', 'Marker3']])
...
def logistic_regression(genomic_data, pca_transformed_data):
    ...
    # Plot the ROC curve
    fpr, tpr, thresholds = sklearn.metrics.roc_curve(genomic_data['Disease'], logreg.predict_proba(pca_transformed_data)[:, 1])
    roc_auc = sklearn.metrics.roc_auc_score(genomic_data['Disease'], logreg.predict_proba(pca_transformed_data)[:, 1])
