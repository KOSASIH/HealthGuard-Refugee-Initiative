...
def prioritize_interventions(classifier, features, priority_type='most_severe'):
   ...
   if priority_type == 'most_severe':
       ...
       sorted_labels = sorted(enumerate(labels), key=lambda x: (x[1], x[0]), reverse=True)
...
prioritized_interventions = prioritize_interventions(classifier, features, priority_type='most_severe')
