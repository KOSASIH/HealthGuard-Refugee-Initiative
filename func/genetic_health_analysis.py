...
def analyze_genetic_data(interim_data):
    ...
    analysis_results['HealthRisks'] = health_risks[['RefugeeID', 'Chromosome', 'Position']].to_dict('records')
    ...
    analysis_results['TreatmentOptions'] = treatment_options[['RefugeeID', 'Chromosome', 'Position']].to_dict('records')
...
def save_analysis_results(analysis_results, data_dir):
    ...
    """
    processed_data_path = os.path.join(data_dir, 'processed', 'genetic_analysis_results.json')
